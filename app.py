...
def evolução_mensal_premium(df):
...
def doughnut_top5_ufs(df, período="all"):
    if período != "all":
        df = df[df["ANO"].astype(int) == int(período)]
    ...
    período_label = str(período) if período != "all" else "2023-2025"
    return _apply_base(fig, height=420, title=f"Top 5 Estados Fornecedores — {período_label}")
...
def _render_tendencias(df, metric="VALOR_PRODUTO"):
    return [
        ...
        dbc.Row([
            dbc.Col(dcc.Graph(figure=evolução_mensal_premium(df), config={"displayModeBar": False}), width=12),
        ], className="g-3 mb-4"),
    ]
    ...
    return [
        ...
        dbc.Row([
            dbc.Col([
                ...
                dcc.Dropdown(id="donut-período", options=anos_opts, value="all",
...
@app.callback(
    ...
    Input("donut-período", "value"),
    Input("filter-ano", "value"), Input("filter-uf", "value"),
    Input("filter-seg", "value"), Input("filter-metric", "value"),
    prevent_initial_call=False,
)
def update_donut(período, ano, uf, seg, metric):
    df = load_enriched_data()
    df_f = apply_filters(df, ano, uf, seg, metric)
    return doughnut_top5_ufs(df_f, período)
